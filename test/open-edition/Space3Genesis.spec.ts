...
describe('Space3Genesis', () => {
  ...
  it('Should deploy successfully', async () => {
    ...
    const startDate = DateTime.fromISO('2024-01-09T00:00:00.000Z').toUnixInteger();
    const endDate = DateTime.fromISO('2024-01-10T00:00:00.000Z').toUnixInteger();
    const space3Genesis = await ethers.deployContract('Space3Genesis', [nftUri, startDate, endDate], {
      signer: owner,
    });
    await space3Genesis.waitForDeployment();
    expect(await space3Genesis.getAddress()).to.be.match(/^0x[a-fA-F0-9]{40}$/);
  });

  it('Should fail to deploy', async () => {
    const { owner } = await loadFixture(deployFixture);
    const startDate = DateTime.fromISO('2024-01-10T00:00:00.000Z').toUnixInteger();
    const endDate = DateTime.fromISO('2024-01-09T00:00:00.000Z').toUnixInteger();
    expect(
  ...
  it('Should fail to mint before public mint period', async () => {
    ...
    await space3Genesis.waitForDeployment();
    expect(space3Genesis.safeMint(owner.address)).to.be.revertedWith('Not in public mint period');
  });
