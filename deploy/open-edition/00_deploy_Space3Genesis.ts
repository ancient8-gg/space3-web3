...
const func: DeployFunction = async function ({ getNamedAccounts, deployments }: HardhatRuntimeEnvironment) {
  ...
  const startTime = DateTime.fromISO(process.env.SPACE3_GENESIS_START_DATE || '').toUnixInteger();
  const endTime = DateTime.fromISO(process.env.SPACE3_GENESIS_END_DATE || '').toUnixInteger();
